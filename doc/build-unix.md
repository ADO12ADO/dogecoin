...
You must install required dependencies to build a basic Nicoin daemon, optional dependencies may vary according to your requirements.
...
Nicoin Core's released binaries are built using the "depends system", which
...
According to installed dependencies, the following steps will compile `nicoind`, `nicoin-cli` and `nicoin-qt`.
...
#### Enable Qt GUI
Create `nicoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Nicoin Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Nicoin Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Nicoin may be compiled in
...
#### Security
To help make your Nicoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Nicoin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Nicoin Core.
