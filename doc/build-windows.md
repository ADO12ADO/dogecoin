...
Below are some notes on how to build Nicoin Core for Windows.
...
Note that for WSL v1 the Nicoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/nicoin, AND not under, for example, /mnt/d/nicoin. 
...
way. This will install to `c:\workspace\nicoin`, for example:
    ...
    make install DESTDIR=/mnt/c/workspace/nicoin
