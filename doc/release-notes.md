Nicoin Core version 1.14.6 is now available from:

  <https://github.com/nicoin/nicoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Nicoin Core users - miners, services,
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/nicoin/lists/nicoin-releases>
...
Nicoin network from 1 NI to 0.01 NI. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 NI as
   change if possible.
2. If after signing there is more than 0.01 NI left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 NI, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 NI will not be accepted as valid by default.
...
```
nicoin-cli addnode <address> remove
```
...
```
nicoin-cli liststucktransactions
```
...
```
nicoin-cli setmaxconnections <desired_amount>
```
...
efforts to make Nicoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Nicoin Core.
