...
Nicoin Core 1.8.1 is primarily a bugfix release, bringing Nicoin Core in
line with Bitcoin 0.9.3. Nicoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Nicoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Nicoin Core clients
* Use HTTPS with nichain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/nicoin/nicoin/issues/134).
...
This only affects values significantly below 1 Ni, however merchants may wish to round to
the nearest Ni on all transactions for now.
